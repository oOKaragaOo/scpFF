    ...
    with sync_playwright() as p:
        ...
        for country in countries:
            ...
            for item in items:
                ...
                airport_name = spans[0].inner_text().strip()
                airport_code = spans[1].inner_text().strip()
                if not airport_code:
                    continue

                # Get the second div for city
                second_div = first_div.query_selector("xpath=following-sibling::div[1]")
                city = ""
                if second_div:
                    text = second_div.inner_text().strip()
                    parts = text.split(',')
                    if len(parts) > 1:
                        city = parts[-1].strip()
